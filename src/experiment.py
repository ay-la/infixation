...
def generate_result(prompt: str, 
                    ...
                    model_name: str):
    '''
    Creates and returns a ModelResponse object to store a text completion generated by an OpenAI model, and the associated information.
...
def get_example_shots(data: List[tuple], 
    ...
    To avoid/minimize oversampling a particular example, examples are selected randomly from the entire data, then compared with a dictionary that keeps track of how many times an example has been sampled in the experiment.
    '''
    i = 0
    examples = []
...
def update_examples_and_dict(example, examples, sampled_dict):
    '''Updates the list of examples given to the model with a new one, and increments the sample counter for that example.'''
...
def run_experiment(num_shots: int, experiment_number: int, experiment_name: str, 
    ...
    Creates an ExperimentResults object to store the text completions for all test words given to the model, and saves the results to a file.
    '''
